...
- 🔴 **修改本文件（CLAUDE.md）必须先经 Wonder 同意**：可以随时提出修改建议和方案，动文件前要获批；Wonder 主动要求的修改除外
...
- **每一步操作让 Wonder 看得懂**：调用工具前用一句中文说明「在做什么、为什么」；工具调用的描述（description）一律用中文写，不用英文
- 产物入 docs/、data/、scripts/，新类型按需建子目录；**知识=知识卡片（结论）+ Zotero（原文件）**，本地缓存 3 天保质
- 多文件修改或方案不明时先说计划再动手；**文档与事实不符，Simon 是第一责任人**：先修再报
- 任务收尾：推翻/补充了**知识卡片** → 当场更新随任务提交；发现**本文件**需要改 → 提出修改方案请示，获批后执行
